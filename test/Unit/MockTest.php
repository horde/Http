...
class MockTest extends TestCase
{
    public function testNoResponses()
    {
        ...
        $client = new Horde_Http_Client(['request' => $mock]);
    ...
    {
        $mock = new Horde_Http_Request_Mock();
        $mock->addResponse('Test', 404, 'http://example.org', ['Test: TEST']);
        $client = new Horde_Http_Client(['request' => $mock]);
        $this->assertEquals('TEST', $client->get()->getHeader('test'));
        $this->assertEquals(['test' => 'TEST'], $client->get()->headers);
    }

    public function testAddStringResponses()
    {
        $mock = new Horde_Http_Request_Mock();
        $mock->addResponses(['A', 'B']);
        $client = new Horde_Http_Client(['request' => $mock]);
        $client->get();
